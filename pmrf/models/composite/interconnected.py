...
class Circuit(Model):
    ...
    models: list[Model] = constrained(init=False, transparent=True)
    indexed_connections: list[list[tuple[int, int]]] = constrained(init=False, static=True)
    port_idxs: list[int] = constrained(init=False, static=True)
...
class Cascade(Model):
    ...
    """
    models: tuple[Model] = constrained(transparent=True)
