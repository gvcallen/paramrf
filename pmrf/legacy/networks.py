...
from legacy.structures import ObservableDict
from legacy.inspection import get_properties_and_attributes
...
def add_noise(network: rf.Network, sigma_gamma=0.1, sigma_tau=None):
    num_freq = network.frequency._npoints
    ...
    for m, n in network.port_tuples:
        if m == n:
            ...
            sigma = sigma_tau
        noise = sigma * _np.random.randn(num_freq) + 1j*sigma * _np.random.randn(num_freq)        
