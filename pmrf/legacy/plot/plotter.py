...
try:
    from anesthetic import make_1d_axes, make_2d_axes, NestedSamples
...
from pmrf.plot import GridFigure
...
class Plotter():
    ...
    def plot_params(self, param_names=None, title='params', label='posterior', priors=False, posteriors=True, fig_size=None, dim='2d', kind='kde', bins=None, fig=None, ax=None):
        if rank != 0:
            return
        ...
        if ax is None:
            if dim == '2d':
                ...
                fig, ax = make_1d_axes(params, figsize=fig_size)
        ...
        if priors:
            ...
            if dim == '2d':
                ...
                prior_samples.plot_1d(ax, label='prior', **kwargs)

        if posteriors:
            if dim == '2d':
                nested_samples.plot_2d(ax, label=label, **kwargs)
            else:
                nested_samples.plot_1d(ax, label=label, **kwargs)
        
        if priors and posteriors:
            if dim == '2d':
                ax.iloc[-1, 0].legend(bbox_to_anchor=(len(ax)/2, len(ax)), loc='lower center', ncol=2)
            else:
                ax.iloc[0].legend(bbox_to_anchor=(len(param_names)/4, 1), loc='lower center', ncol=2)
