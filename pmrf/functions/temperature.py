...
# To be implemented. Needs skrf.connect as currently written
# def output_temperature_alternative(source: Model, passive: Model, Ts, Tp):   
...
#     load = Model(frequency=source.frequency, s=np.conj(gamma_2), z0=passive.z0[:,1])
#     gamma_L = load.s[:,0,0]
...
# def cascaded_output_temperature(source: Model, passives: list[Model], Ts, temperatures):
...
#     load = Model(frequency=source.frequency, s=np.conj(gamma_backwards[-1]), z0=passive.z0[:,1])
#     gamma_L = load.s[:,0,0]
