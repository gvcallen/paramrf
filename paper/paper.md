...
- **Built-in Solvers:** The framework provides high-level sub-modules (`pmrf.optimize`, `pmrf.infer`, `pmrf.fitting`) that abstract different solver backends. Users can easily swap between classical optimization (using SciPy or Optimistix) and Bayesian inference (using BlackJAX) to perform parameter estimation and uncertainty quantification.
