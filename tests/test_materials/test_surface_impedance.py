...
def test_half_space_is_the_trivial_shape_factor(freq):
    ...
    zs = HalfSpaceSurfaceImpedance().impedance(freq.w, conductor)
...
def test_holloway_kuester_slab_matches_half_thickness_coth():
    ...
    actual = HollowayKuesterSlabSurfaceImpedance().impedance(freq.w, conductor, t=t)
...
def test_holloway_kuester_slab_has_dc_and_strong_skin_limits():
    ...
    zs = HollowayKuesterSlabSurfaceImpedance().impedance(freq.w, conductor, t=t)
...
def test_root_sum_square_slab_blends_resistance_and_keeps_half_space_reactance():
    ...
    zs = RootSumSquareSlabSurfaceImpedance().impedance(freq.w, conductor, w=w, t=t, weight=weight)
...
def test_root_sum_square_slab_overstates_internal_inductance_below_one_skin_depth():
    ...
    exact_x = jnp.imag(HollowayKuesterSlabSurfaceImpedance().impedance(freq.w, conductor, t=t))
...
def test_tabulated_planar_entries_record_transition_error(w, h, zc, expected, rtol):
    ...
    actual = jnp.stack(
        [
            jnp.real(
                HollowayKuesterSlabSurfaceImpedance().impedance(freq.w, conductor, **geometry)
                * weight
            ),
            jnp.real(HalfSpaceSurfaceImpedance().impedance(freq.w, conductor, **geometry) * weight),
...
def test_tesche_rod_matches_its_own_strong_skin_asymptote():
    ...
    zs = TescheRodSurfaceImpedance().impedance(freq.w, conductor, a=a)
...
def test_tesche_tube_matches_its_own_strong_skin_asymptote():
    ...
    zs = TescheTubeSurfaceImpedance().impedance(freq.w, conductor, a=a, t=t)
...
def test_tesche_tube_approaches_half_space_as_the_wall_thickens():
    """A very thick tube is a half-space: TescheTubeSurfaceImpedance's own limit.
    ...
    thick_tube = TescheTubeSurfaceImpedance().impedance(freq.w, conductor, a=a, t=1e6)
    half_space = HalfSpaceSurfaceImpedance().impedance(freq.w, conductor)
...
def test_schelkunoff_rod_matches_scipy_bessel_ratio():
    ...
    zs = SchelkunoffRodSurfaceImpedance().impedance(freq.w, conductor, a=a)
...
def test_schelkunoff_rod_reproduces_the_dc_resistance_without_a_special_case():
    ...
    z = SchelkunoffRodSurfaceImpedance().impedance(freq.w, conductor, a=a) / (2 * jnp.pi * a)
...
def test_schelkunoff_rod_tends_to_the_half_space_plus_curvature():
    ...
    This curvature term is exactly what :class:`TescheRodSurfaceImpedance` cannot
...
def test_tesche_rod_departs_from_schelkunoff_by_a_shape_error():
    ...
    tesche = TescheRodSurfaceImpedance().impedance(freq.w, conductor, a=a)
    exact = SchelkunoffRodSurfaceImpedance().impedance(freq.w, conductor, a=a)
...
def test_schelkunoff_rod_gradients_are_finite_including_at_dc():
    ...
    def resistance(sigma, f):
        ...
        return jnp.real(SchelkunoffRodSurfaceImpedance().impedance(freq.w, conductor, a=a))[0]
...
def test_schelkunoff_rod_is_lossless_for_a_perfect_conductor():
    ...
    zs = SchelkunoffRodSurfaceImpedance().impedance(freq.w, conductor, a=0.455e-3)
...
def test_schelkunoff_tube_matches_scipy_evaluation_of_eq_74(f, t, rtol):
    ...
    zs = SchelkunoffTubeSurfaceImpedance().impedance(freq.w, conductor, a=a, t=t)
...
def test_schelkunoff_tube_infinite_wall_is_the_infinite_wall_limit():
    ...
    infinite = SchelkunoffTubeSurfaceImpedance().impedance(freq.w, conductor, a=a, t=jnp.inf)
    ...
    thick = SchelkunoffTubeSurfaceImpedance().impedance(freq.w, conductor, a=a, t=30e-3)
...
def test_schelkunoff_tube_gradients_are_finite_at_dc_and_for_a_perfect_conductor():
    ...
    def resistance(sigma, f):
        freq = Frequency.from_f(jnp.atleast_1d(f))
        conductor = BulkConductor(sigma=sigma).properties(freq)
        return jnp.real(SchelkunoffTubeSurfaceImpedance().impedance(freq.w, conductor, a=a, t=t))[0]
    ...
    for f in (0.0, 1e3, 1e9):
        ...
        assert jnp.all(
            SchelkunoffTubeSurfaceImpedance().impedance(freq.w, conductor, a=a, t=t) == 0
        )
        wall_gradient = jax.grad(
            lambda wall: jnp.real(
                SchelkunoffTubeSurfaceImpedance().impedance(freq.w, conductor, a=a, t=wall)
            )[0]
        )(t)
...
@pytest.mark.parametrize(
    'shape, geometry',
    [
        (HollowayKuesterSlabSurfaceImpedance(), dict(t=18e-6)),
        (SchelkunoffRodSurfaceImpedance(), dict(a=20e-6)),
        (SchelkunoffTubeSurfaceImpedance(), dict(a=1.475e-3, t=25e-6)),
        (SchelkunoffTubeSurfaceImpedance(), dict(a=1.475e-3, t=jnp.inf)),
    ],
)
...
def test_even_odd_slab_reproduces_the_true_dc_resistance_under_any_weight():
    """The even mode carries 1/(sigma*W*t) into whatever weight the caller has.

    The mixing fraction alpha = 1/(2*W*k) is exactly what cancels the caller's
    weight at dc, so the floor is independent of k. Checked at Cohn's
    stripline weight for a 2.655 mm strip (435.27 1/m, alpha = 0.4327) and at
    the even mode's own weight 1/(2W) (alpha = 1), which are a factor 2.31
    apart. Exact arithmetic, so rtol is at double precision.
    """
    sigma, w, t = 5.8e7, 2.655e-3, 35e-6
    freq = Frequency.from_f(jnp.array([0.0]))
    conductor = _conductor(freq, sigma)

    for weight in (435.26525504, 1 / (2 * w)):
        zs = EvenOddSlabSurfaceImpedance().impedance(
            freq.w, conductor, w=w, t=t, weight=weight
        )
        assert jnp.allclose(jnp.real(zs * weight), 1 / (sigma * w * t), rtol=1e-12)
        assert jnp.allclose(jnp.imag(zs), 0.0)


def test_even_odd_slab_reaches_the_half_space_under_strong_skin_effect():
    """Both modes tend to zeta_c, and the coefficients sum to 1 by construction.

    At 10 GHz in copper, t/delta = 53 for 35 um, so coth and tanh differ from
    1 by exp(-2 t/delta) ~ 1e-46: the agreement is limited by the double
    precision of the arithmetic, not by the asymptote.
    """
    sigma, w, t, weight = 5.8e7, 2.655e-3, 35e-6, 435.26525504
    ...
    assert jnp.allclose(zs, conductor.zs, rtol=1e-12)


def test_even_odd_slab_is_the_exact_slab_when_the_weight_is_its_own():
    """At alpha = 1 the odd mode drops out and only the exact even mode is left.

    This is what makes the mix a normalisation device rather than a new
    physical model: given the even mode's own weight 1/(2W) it reduces to
    HollowayKuesterSlabSurfaceImpedance identically, across the transition and
    not only at the limits.
    """
    sigma, w, t = 5.8e7, 2.655e-3, 35e-6
    freq = Frequency.from_f(jnp.array([1e5, 1e7, 1e9]))
    conductor = _conductor(freq, sigma)

    mixed = EvenOddSlabSurfaceImpedance().impedance(
        freq.w, conductor, w=w, t=t, weight=1 / (2 * w)
    )
    exact = HollowayKuesterSlabSurfaceImpedance().impedance(freq.w, conductor, t=t)

    assert jnp.allclose(mixed, exact, rtol=1e-12)


def test_even_odd_slab_takes_internal_reactance_off_the_semi_infinite_law():
    """#123: below the knee X goes as omega, not as sqrt(omega).

    Measured against the exact slab on the stripline default geometry
    (2.655 mm strip, 35 um copper, Cohn's weight 435.27 1/m, alpha = 0.4327).
    At 100 kHz, t/delta = 0.167:

    | model                  | X / X_exact |
    |------------------------|-------------|
    | even-odd mix           | 4.93        |
    | root-sum-square blend  | 41.4        |

    The mix is not exact -- the odd mode contributes omega*mu*t/2 at a
    fraction fixed by the resistance asymptotes, not by mode excitation -- so
    the number recorded is 4.93, not 1. What has changed is the law: halving
    the frequency halves the mix's reactance and divides the blend's by
    sqrt(2). The ratios are held to 1e-3, which is well inside the spread of
    either model and is here only to pin the recorded values.
    """
    sigma, w, t, weight = 5.8e7, 2.655e-3, 35e-6, 435.26525504
    freq = Frequency.from_f(jnp.array([5e4, 1e5]))
    conductor = _conductor(freq, sigma)
    kwargs = dict(w=w, t=t, weight=weight)

    mixed = EvenOddSlabSurfaceImpedance().impedance(freq.w, conductor, **kwargs)
    blend = RootSumSquareSlabSurfaceImpedance().impedance(freq.w, conductor, **kwargs)
    exact = HollowayKuesterSlabSurfaceImpedance().impedance(freq.w, conductor, t=t)
    exact = exact / (2 * w * weight)

    assert jnp.allclose(jnp.imag(mixed)[1] / jnp.imag(exact)[1], 4.9337, rtol=1e-3)
    assert jnp.allclose(jnp.imag(blend)[1] / jnp.imag(exact)[1], 41.401, rtol=1e-3)
    # The law itself: doubling f doubles X for the mix and the exact slab,
    # and multiplies the half-space-reactance blend by sqrt(2).
    assert jnp.allclose(jnp.imag(mixed)[1] / jnp.imag(mixed)[0], 2.0, rtol=1e-3)
    assert jnp.allclose(jnp.imag(exact)[1] / jnp.imag(exact)[0], 2.0, rtol=1e-3)
    assert jnp.allclose(jnp.imag(blend)[1] / jnp.imag(blend)[0], jnp.sqrt(2.0), rtol=1e-6)


def test_even_odd_slab_is_finite_for_a_perfect_conductor():
    """sigma -> inf has no dc floor and no diffusion; both branches are guarded."""
    freq = Frequency.from_f(jnp.array([0.0, 1e9]))
    conductor = _conductor(freq, jnp.inf)

    zs = EvenOddSlabSurfaceImpedance().impedance(
        freq.w, conductor, w=2.655e-3, t=35e-6, weight=435.26525504
    )

    assert jnp.all(jnp.isfinite(zs))
    assert jnp.allclose(zs, 0.0)
