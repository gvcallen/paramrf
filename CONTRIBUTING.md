...
* **RF Models** (`pmrf.models`). These are the main, built-in RF models in the library. They have been separated into an deep organizational structure (e.g. lumped components, surrogates etc.). If a model's forward pass has many different possible implementations (for example, different models for a coaxial line, or different circuit solver algorithms) then a separate strategy class should be created alongside the model which implements some abstract interface (referred to as the "strategy" pattern, described below). See `pmrf.models.CoaxialLine` and `pmrf.models.Circuit` for simple and complex examples, respectively.
...
* Anything that actually solves a system, e.g. by inverting a matrix or iterating to convergence, is a "solver", and should be referred to as such. If a specific type of a solver has a well-defined name, e.g. an "optimizer", this name can be used for the class name, but user-facing variables should still be called "solver".
* A strategy that instead evaluates a closed-form expression is a "formulation", e.g. `TescheCoaxialFormulation`, and the field holding it is called `formulation`. Formulations take already-evaluated arrays rather than `Param` or `Module` objects, so they can be checked directly against the equations of the paper they come from.
* Parameter names follow the **symbol test**: if the quantity appears as a symbol in the class's `**Mathematical Formulation**` section, name the field after that symbol, transliterating subscripts with underscores, e.g. $\varepsilon_{eff}$ becomes `ep_eff` and $d_{in}$ becomes `d_in`. Otherwise name it in explicit English, e.g. `length`, `roughness`, `dielectric`. The docstring section is mandatory for physics classes, so this is decidable rather than a matter of taste.
* Two names are single words by established convention and keep their spelling: `tand` (as in scikit-rf) and `lengthscale` (as in the Gaussian-process literature, where it is an English compound rather than a compressed symbol). Bare single-letter symbols such as `w`, `h`, `t`, `R`, `L`, `G`, `C`, `Z`, `Y` also stay as they are.
