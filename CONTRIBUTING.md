...
* Two names are single words by established convention and keep their spelling: `tand` (as in scikit-rf) and `lengthscale` (as in the Gaussian-process literature, where it is an English compound rather than a compressed symbol). Bare single-letter symbols such as `w`, `h`, `t`, `R`, `L`, `G`, `C` also stay as they are.
