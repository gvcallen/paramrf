...
In general, ParamRF follows a tiered API to make it easy for users will isolating the JAX and solver logic (make sure to have a basic grasp on JAX PyTrees and perhaps the `Equinox` library before continuing). Higher-level methods should always be available to perform a task with minimal imports. However, these should rely on mid-tier functions in the library that use "glue" helper classes which power-users can also use.

### The Module Split
* **Core Primitives** (`pmrf.Model`, `pmrf.Frequency`, `pmrf.parameters`). These are the core primitives used throughout the library. The represent the core classes and factory functions users will interact with, and build on top of the `Equinox` and `Parax` JAX libraries.
